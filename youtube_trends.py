...
def fetch_youtube_trends_with_progress(keywords, timeframe, batch_size=2, delay=60):
    ...
    for i, batch_start in enumerate(range(0, len(keywords), batch_size), 1):
        ...
        try:
            ...
            pytrends.build_payload(batch, timeframe=timeframe, geo='US', gprop='youtube')
...
if __name__ == "__main__":
    ...
    if top_symbols:
        ...
        trends_1day = fetch_youtube_trends_with_progress(top_symbols, 'now 1-d', batch_size=2, delay=60)
        print("\nFetching 7-day YouTube Trends data...")
        trends_7days = fetch_youtube_trends_with_progress(top_symbols, 'now 7-d', batch_size=2, delay=60)
        print("\nFetching 30-day YouTube Trends data...")
        trends_30days = fetch_youtube_trends_with_progress(top_symbols, 'today 1-m', batch_size=2, delay=60)
        ...
        # 각각 결과 출력
        print("\nTop 10 YouTube Search Volume Rankings (1-day):")
        print(trends_1day)

        print("\nTop 10 YouTube Search Volume Rankings (7-day):")
        print(trends_7days)

        print("\nTop 10 YouTube Search Volume Rankings (30-day):")
        print(trends_30days)

        # 결과 저장
        trends_1day.to_csv("youtube_trends_1day_top10.csv", index=False)
        trends_7days.to_csv("youtube_trends_7days_top10.csv", index=False)
        trends_30days.to_csv("youtube_trends_30days_top10.csv", index=False)
        print("\nResults saved to 'youtube_trends_1day_top10.csv', 'youtube_trends_7days_top10.csv', and 'youtube_trends_30days_top10.csv'")
